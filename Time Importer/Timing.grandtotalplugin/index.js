...
function sendEstimate() {
	...
	var quotedAndCommaSeparated =
		'{"' + parameters.map((param) => param.replace(/\\/g, "\\\\").replace(/"/g, '\\"')).join('","') + '"}';
...
function timeEntriesAPI() {
	var url =
		"https://web.timingapp.com/api/v1/time-entries?is_running=false&include_project_data=true&include_team_members=true&start_date_min=2010-01-01&start_date_max=2050-01-01&billing_status[]=billable&billing_status[]=billed&billing_status[]=paid&billing_status[]=undetermined";
	var counter = 0;
	var result = [];
	...
	while (url && counter < 5) {
		...
		for (itemToParse of items) {
			if (
				...
				(!itemToParse["project"] || !itemToParse["project"]["title_chain"].join(" ▸ ").includes(filterString))
			) {
				continue;
			}
...
function parseComponents(rawComponents) {
	...
	if (projectComponents.length == 1) {
		...
		result["client"] = localize("Move project '%' in a folder named after your client").replace(
			"%",
			lineitem["project"]
		);
	} else {

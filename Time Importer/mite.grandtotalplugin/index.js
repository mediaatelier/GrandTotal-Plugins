...
function httpPostJSON(theUrl, body) {
	...
	header = {
		Authorization: "Basic " + base64Encode(token.trim() + ":api_token"),
...
function getPage(page) {
	...
	return httpGetJSON(
		"https://" +
			...
			".mite.de/time_entries.json?from=" +
			aStartDateString +
			"&project_id=all_active&limit=1000&sort=date&page=" +
			page
	);
}
...
function importTimedEntries() {
	...
	for (page = 1; page < 21; page++) {
		...
		for (var i = 0; i < aArray.length; i++) {
			...
			aItemResult["url"] =
				"https://" + accountName + ".mite.de/daily#" + datePart + "?open=time_entry_" + aEntry["id"];
...
function exportTimedEntries() {
	...
	for (i in timeentries) {
		...
		service = getRecordForNameInEndpoint(
			services,
			serviceName,
			"services",
			"service",
